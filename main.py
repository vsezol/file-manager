...
class MainWidget(Screen):
    ...
    def draw_files(self, files, select_callback):
        ...
        for file in files:
            ...
            fileblock = None
            if file_type == 'file':
                fileblock = FileBlock(text=file, background_color = (0.5, 0., 1., 1), on_release=select_callback)
            elif file_type == 'dir':
                fileblock = FileBlock(text=file, background_color=(0, 0.5, 0.5, 1), on_release=select_callback)
...
class FileManagerApp(App):
    ...
    def delete_file(self):
        file, ftype = self.selcted_file
        try:
            if ftype == 'dir':
                shutil.rmtree(file)
            elif ftype == 'file':
                os.remove(file)
            self.dirs.remove(file)
        except:
            pass
        self.update_files()
    ...
    def select_file(self, instance):
        self.selcted_file = [instance.text, check_file_type(instance.text)]
    ...
    def update_files(self):
        self.mw.draw_files(self.dirs, self.select_file)
